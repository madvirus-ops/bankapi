...
import schemas
import models
from utils import SECRET_KEY,access_cookies_time,ACCESS_TOKEN_LIFETIME_MINUTES,ALGORITHM,refresh_cookies_time,REFRESH_TOKEN_LIFETIME
...
@router.post("/login")
async def log_user_in(request:schemas.Login,response:Response,db:Session = Depends(get_db),Authorize:AuthJWT = Depends()):
